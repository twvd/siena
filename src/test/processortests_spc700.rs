...
macro_rules! _cpu_test {
    ($testfn:ident, $instr:expr, $trace:expr, $steps:expr) => {
        ...
        fn $testfn() {
            ...
            let filename = format!(
                "../siena_tests/ProcessorTests/spc700/v1/{:02x}.json",
                $instr
            );
            let testcases: Value =
...
fn parse_ram(v: &Value) -> HashMap<SpcAddress, u8> {
...
fn run_testcase(testcase: &Value, check_trace: bool, multi_steps: bool) {
    ...
    let mut bus = Testbus::<SpcAddress>::new(SPC_ADDRESS_MASK);
    for (addr, val) in ram_initial {
        bus.write(addr, val);
    }

    let mut cpu = CpuSpc700::<Testbus<SpcAddress>>::new(bus, 0);
    ...
    // verification later.
    let bus = std::mem::replace(&mut cpu.bus, Testbus::<SpcAddress>::new(SPC_ADDRESS_MASK));
...
// Read indirection during fetch
cpu_test_no_trace!(instr_99, 0x99);
...
// Read indirection during fetch
cpu_test_no_trace!(instr_fa, 0xfa);
